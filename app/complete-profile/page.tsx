...
export default function CompleteProfilePage() {
  ...
  const loadVerificationData = useCallback(async () => {
    try {
      ...
      if (verificationData) {
        ...
        setFormData(prev => ({
          ...prev,
          businessName: parsedData.supplier?.name || parsedData.supplier?.profile?.businessName || prev.businessName,
          contactEmail: parsedData.supplier?.email || parsedData.supplier?.profile?.contactEmail || prev.contactEmail,
          contactPhone: parsedData.supplier?.phone || parsedData.supplier?.profile?.mainPhone || prev.contactPhone,
        }));
      ...
      setFormData(prev => ({
        ...prev,
        contactEmail: profileData.contactEmail || profileData.email || prev.contactEmail,
        contactPhone: profileData.contactPhone || profileData.phone || prev.contactPhone,
  ...
  useEffect(() => {
    if (formData.location.lat !== selectedLocation.lat || formData.location.lng !== selectedLocation.lng) {
      setFormData(prev => ({
        ...prev,
              ...
              {Array.from({ length: TOTAL_STEPS }, (_, i) => i + 1).map((step) => {
                ...
                return (
                  <button
                    ...
                    title={isAccessible ? `Go to step ${step}` : `Complete step ${step - 1} first`}
                  >
                    {step < currentStep ? (
